"""Split OAS aerostructural analysis with a large CRM wing mesh.

Same analysis as the small-mesh ``oas_aerostruct_split`` example but with
21 spanwise and 7 chordwise panels for higher fidelity.
...
def run(
    ...
    """Run a split OAS aerostructural analysis (large mesh).
    ...
    servers = []
    if start_servers:
        servers.append(_start_server(OasGeomDiscipline(mesh_dict=MESH_DICT), GEOM_PORT))
        servers.append(
            _start_server(OasSplitAeroDiscipline(mesh_dict=MESH_DICT), AERO_PORT)
        )
        servers.append(
            _start_server(OasSplitStructDiscipline(mesh_dict=MESH_DICT), STRUCT_PORT)
        )
    ...
    try:
        ...
        ny = MESH_DICT["num_y"]
        if MESH_DICT.get("symmetry", False):
            ny = (ny + 1) // 2
        ...
        print(
            f"Split OAS aerostructural analysis (large mesh): v={v} m/s, "
            f"alpha={alpha} deg, M={mach_number}, Re={re:.0e}/m, "
            f"rho={rho} kg/m^3"
        )
        print(f"  CL      = {cl:.4f}")
        print(f"  CD      = {cd:.6f}")
